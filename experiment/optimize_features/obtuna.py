...
async def generate_dataset_from_paths(parent_dir: str, sql_dump_paths: List[str], output_path: str, extractor_name: str, mode: str):
    """
...
def load_best_params(dataset_path: str = "./experiment/optimize_features/dataset_cache.pkl") -> dict:
    save_path = os.path.join(os.path.dirname(dataset_path), "best_params.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="./experiment/optimize_features/dataset_cache.pkl", help="Path to dataset pickle.")
    parser.add_argument("--trials", type=int, default=100, help="Number of trials.")
    parser.add_argument("--mode", type=str, default="mobilenet", choices=["mobilenet", "vertex", "gps_time"], 
                        help="Optimization mode.")
    
    # Generation args
    parser.add_argument("--generate-from", type=str, help="Parent directory to recursively search for images.")
    parser.add_argument("--sql-dump", nargs='+', help="List of paths to SQL dumps for ground truth labels.")
    
    args = parser.parse_args()
    
    if args.generate_from:
        if not args.sql_dump:
            print("Warning: --sql-dump not provided. Dataset will have no labels (useful only for inference tests, not optimization).")
        
        # Use asyncio to run the generator
        dataset_path = asyncio.run(generate_dataset_from_paths(
            parent_dir=args.generate_from,
            sql_dump_paths=args.sql_dump,
        ...
        if not os.path.exists(dataset_path):
            print(f"Dataset {dataset_path} not found. Use --generate-from to create one.")
            exit(1)
